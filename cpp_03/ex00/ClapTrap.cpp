...
{
    if (hitPoints <= 0){
        std::cout << "ClapTrap의 체력(hit point)이 0입니다." << std::endl;
        return ;
    }
    if (energyPoints >= 1){
...
void ClapTrap::takeDamage(unsigned int amount){
    std::cout << "ClapTrap " << name << "이 " << amount << " 포인트의 피해를 입었습니다!" << std::endl;
    hitPoints -= amount;
    if (hitPoints < 0)
        hitPoints = 0;
}
...
void ClapTrap::beRepaired(unsigned int amount){
    ...
    if (energyPoints >= 1){
        std::cout << "ClapTrap " << name << "이 " << amount << " 포인트의 피해를 수리하여 체력이 회복되었습니다!" << std::endl;
        hitPoints += amount;
        energyPoints--;
    }
    else {
